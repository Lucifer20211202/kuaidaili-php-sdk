...
class Client
{
     ...
     *    )
     * @return string 订单过期时间字符串
     */
    public function getExpireTime($args = [])
    {
     ...
     *           )
     * @return array 返回信息的数组
     */
    public function getProxyAuthorization($args = [])
    {
     ...
     * @return:返回ip地址。
     */
    public function tpsCurrentIp($args = [])
    {
     ...
     * @return: 返回新的IP地址
     */
    public function changeTpsIp($args = [])
    {
     ...
     */
    public function getTps($args = [])
    {

        # 设置默认值
        $args["sign_type"] = isset($args["sign_type"]) ? $args["sign_type"] : "token";
        if (!isset($args["num"])) {
            throw new KdlNameError("miss param: num");
        }
        if (!ctype_digit(strval($args["num"]))) {
            throw new KdlTypeError("num should be a integer or a integer string");
        }
     ...
     * @return ip白名单列表
     */
    public function getIpWhiteList($args = [])
    {
     ...
     */
    public function checkDpsValid($args = [])
    {
        ...
        if (!isset($args["proxy"])) {
            throw new KdlNameError("miss param: proxy");
        }
        if (!is_array($args["proxy"])) {
            throw new KdlTypeError("args['proxy'] should be a array");
        }
    ...
    public function getDpsValidTime($args = [])
    {
        /*
            获取私密代理ip有效时间
            :param args
     ...
     */
    public function getIpBalance($args = [])
    {
    ...
    public function getProxy($args = [])
    {
        ...
        if ($endpoint == OpsOrderLevel::SVIP) {
            $endpoint = EndPoint::GetOpsProxySvip;
        }
        if ($endpoint == OpsOrderLevel::PRO) {
            $endpoint = OpsOrderLevel::PRO;
        }
     ...
     */
    public function checkOpsValid($args = [])
    {
     ...
     */
    public function setIpWhiteList($args = [])
    {
        ...
        if (isset($args["iplist"])) {
            $args["iplist"] = implode(",", $args["iplist"]);
        }
        $args["sign_type"] = isset($args["sign_type"]) ? $args["sign_type"] : "token";
     ...
     * @return User-Agent列表
     */
    public function getUA($args = [])
    {
        $endpoint = EndPoint::GetUA;
        if (isset($args) && !(is_array($args))) {
            echo "args should be a array \n";
            return false;
        }
     ...
     * @return 地区编码信息关联列表
     */
    public function getAreaCode($args = [])
    {
     ...
     * @return Json
     */
    public function setAutoRenew($args = [])
    {
     ...
     * @return Json
     */
    public function queryKpsCity($args = [])
    {
    ...
    function _getSecretToken()
    {
        ...
        $args = ["sign_type" => "simple", "secret_key" => $this->auth->secretKey];
        $params = $this->_get_params($endpoint, $args);
        $res = $this->_get_base_res(
            "POST",
            $endpoint,
        ...
        return [$res["data"]["secret_token"], $res["data"]["expire"], time()];
    }
    ...
    public function getSecretToken()
    {
        try {
            if (file_exists($this->secretPath)) {
                $token = $this->_readSecretToken();
            } else {
                $token = $this->_writeSecretToken()[0];
            }
            return $token;
    ...
    private function _get_base_res($method, $endpoint, $query)
    {
        ...
        # 非200状态码，抛出异常
        if ($status_code != 200) {
            throw new KdlStatusError($message = $r, $code = $status_code);
        }
...
//        var_dump($res);
        ...
        $msg = $res['msg'];
        if ($code != 0) {
            throw new KdlException($msg, $code);
        }
     ...
     * @return string
     */
    private function _get_params($endpoint, $args = [])
    {
        ...
        switch ($params["sign_type"]) {
            ...
            case "hmacsha1":
                ...
                $params["signature"] = $this->auth->signStr($raw_str, "sha1");
                break;
            default:
